...
function updateStats() {
    const counts = {
        uml: allArrests.filter(a => a.party === 'CPN-UML').length,
        nc: allArrests.filter(a => a.party === 'Nepali Congress').length,
        maoist: allArrests.filter(a => a.party === 'Maoist Centre').length,
        ind: allArrests.filter(a => a.party === 'Independent').length,
        total: allArrests.length
    };

    document.getElementById('umlScore').innerText = counts.uml;
    document.getElementById('ncScore').innerText = counts.nc;
    document.getElementById('maoistScore').innerText = counts.maoist;
    document.getElementById('indScore').innerText = counts.ind;
    document.getElementById('totalScore').innerText = counts.total;
}
    ...
    grid.innerHTML = data.map(item => `
        <div class="ok-card">
            <div class="flex justify-between items-start mb-6">
                <span class="text-[9px] font-black px-2 py-1 rounded bg-slate-100 ${item.party === 'CPN-UML' ? 'text-red-600' : item.party === 'Nepali Congress' ? 'text-green-600' : 'text-slate-500'} uppercase">
                    ${item.party}
                </span>
            ...
            <div class="pt-4 border-t border-slate-100 mt-auto">
                <p class="text-[10px] text-slate-400 italic mb-6 leading-relaxed">"${item.reason}"</p>
...
function googleTranslateElementInit() {
    new google.translate.TranslateElement({pageLanguage: 'en', includedLanguages: 'en,ne', autoDisplay: false}, 'google_translate_element');
}

document.getElementById('langToggle').addEventListener('click', function() {
    const googleCombo = document.querySelector('.goog-te-combo');
    if (googleCombo) {
        googleCombo.value = (this.innerText === 'नेपाली') ? 'ne' : 'en';
        this.innerText = (this.innerText === 'नेपाली') ? 'English' : 'नेपाली';
